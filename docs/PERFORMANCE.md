...
| 설정 | 피크 RSS | p50 | 비고 |
|---|---:|---:|---|
| **기본 (rerank on, int8, `POOL=20`)** | 3,095MB | **~350–410ms** | 현재 기본값 |
| `RERANK_POOL=60` (옛 기본값) | 3,095MB | ~900ms | 2.5배 느리고 **정확도도 낮음** — 아래 참고 |
...
### `RERANK_POOL` — 키우면 오히려 나빠진다 (기본값 60 → 20 변경)

cross-encoder 에 넘길 후보 수. 직관과 달리 **많이 줄수록 정확도가 떨어진다**.
FTS5 상위는 이미 잘 정렬돼 있고, 아래쪽을 끌어올리면 재정렬이 순위를 흐트러뜨리기 때문.

| pool | 5 | 10 | **20** | 40 | 60(옛 기본) | 120 |
|---|---|---|---|---|---|---|
| 룰기반 골든 300건 | 226 | 226 | **200** | 174 | 169 | 163 |
| LLM 자연어 골든 43건 | 30 | 30 | **35** | – | 34 | – |
| p50 지연 | 46s | 46s | **90s** | 164s | 234s | 346s |

*(지연은 300케이스 전체 소요)*

**20 을 기본값으로 채택한 근거** — 두 골든셋 **모두에서** 옛 기본값 60 보다 우세하다.
- 룰기반 60 vs 20: McNemar **p=0.0000** (5 : 36) — 유의
- LLM 60 vs 20: p=1.0 (3 : 4) — 동등하나 방향은 20 쪽
- 속도 2.5배

**5/10 은 채택하지 않는다** — 룰기반에서는 최고(226)지만 LLM 자연어에서는 최악(30)으로
**방향이 정반대**다. 키워드 나열 질의에만 유리한 골든셋 아티팩트이므로, 실사용에서는
오히려 손해일 수 있다. (`docs/BENCHMARK_METHODOLOGY.md` 기법 3의 교차검증 규칙 적용 사례)
